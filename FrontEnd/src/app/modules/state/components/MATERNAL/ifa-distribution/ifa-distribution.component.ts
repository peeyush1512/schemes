...
@Component({
  selector: 'app-ifa-distribution',
  templateUrl: './ifa-distribution.component.html',
  styleUrls: ['./ifa-distribution.component.scss']
})
export class IFADistributionComponent {
 ...
 calldata() {
    ...
    this.apiservice.callapi('ifad').subscribe((result:any)=>{     
      result.forEach((element:any) => {     
        if (element.ifad > 90) {
          this.data[0].data.push(element)
          
        } else if (element.ifad <= 90 && element.ifad >= 75) {
          this.data[1].data.push(element)
          
        } else if (element.ifad < 75) {
