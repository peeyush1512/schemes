...
@Component({
  selector: 'app-pw-received-tsz-calcium-tablet',
  templateUrl: './pw-received-tsz-calcium-tablet.component.html',
  styleUrls: ['./pw-received-tsz-calcium-tablet.component.scss']
})
export class PWReceivedTSZCalciumTabletComponent {
  ...
  calldata() {
    ...
    this.apiservice.callapi('pwr360c').subscribe((result:any)=>{     
      result.forEach((element:any) => {     
        if (element.pwrc > 90) {
          this.data[0].data.push(element)
          
        } else if (element.pwrc <= 90 && element.pwrc >= 75) {
          this.data[1].data.push(element)
          
        } else if (element.pwrc < 75) {
